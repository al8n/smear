//! GraphQLx selection AST node types: aliases, fields, selections, fragments, and
//! selection sets.
//!
//! Copied type-only from the frozen `smear-parser` crate
//! (`graphqlx/ast/default/field.rs` and `default/fragment_definition.rs`): the node
//! types the selection productions return, keyed by the source slice `S` with spans
//! pinned to [`SimpleSpan`](tokora::SimpleSpan). Two GraphQLx twists over the
//! GraphQL shapes: a fragment spread names a [`FragmentTypePath`] (a `::`-path with
//! optional generic *arguments*, `...ns::Frag<T>`), and a type condition targets a
//! generic [`TypePath`](scaffold::generic::TypePath) (`on Item<T>`), not a bare
//! name. The `Field` newtype and `Selection` sum are real types (not aliases) so
//! the recursion `Selection → Field → SelectionSet → Selection` terminates at
...
use super::{Arguments, Directives, Name, Type};

/// A field alias in a selection set (`Name :`).
pub type Alias<S> = scaffold::Alias<Name<S>>;

/// A type path for a fragment reference, with optional generic arguments
/// (`ns::Frag<T>`).
pub type FragmentTypePath<S> = scaffold::generic::FragmentTypePath<Name<S>, Type<S>>;

/// A type condition for a fragment: the (possibly generic) type path it applies to
/// (`on Item<T>`).
pub type TypeCondition<S> = scaffold::TypeCondition<scaffold::generic::TypePath<Name<S>, Type<S>>>;

/// A fragment spread in a selection set (`... FragmentTypePath Directives?`).
