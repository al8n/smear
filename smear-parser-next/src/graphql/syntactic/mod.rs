...
///
/// The one-token, non-consuming dispatch primitive the selection and executable
/// productions reach for when they must decide a fork on the shape of the next
/// token — a field vs a `...`, a shorthand `{` vs an operation keyword — while
/// leaving that token in place for the committed arm. Mirrors the closure discipline
/// of [`value`](value::value)'s `classify_value_head`.
