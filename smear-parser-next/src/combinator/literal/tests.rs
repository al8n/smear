// Literal atoms need `LiteralValueToken`, which among the dialect lexers only
// GraphQL's `SyntacticToken` supplies today (GraphQL-like literal semantics are a
// radix superset with no capability impl yet), so the literal tests drive GraphQL
// alone. Every atom runs over the full source matrix (`str`, `[u8]`, and `Bytes`).
// The atoms are declining-only — they consume nothing on a miss and never emit a
// diagnostic — so the accept and decline paths need a single emitter mode; the
// harness stays emitter-parametrized to match the token atoms' harness.
#![cfg(feature = "graphql")]
...
use super::{StringLiteral, try_block_str, try_float, try_inline_str, try_int, try_string};

use crate::combinator::ident;
...
/// [`Fatal`] instantiates as a complete emitter over it, and the
...
// carrier's raw source slice via `source_ref`, not the unescaped text). Decline
// paths assert the leftover's TYPE by parsing it with the atom that matches it,
// which also proves the decline consumed nothing.
