...
pub struct WherePredicate<Ident, Type, Container = Vec<TypePath<Ident, Type>>> {
  span: Span,
  bounded_type: TypePath<Ident, Type>,
  bounds: Container,
}
...
impl<Ident, Type, Container> IntoComponents for WherePredicate<Ident, Type, Container> {
  type Components = (Span, TypePath<Ident, Type>, Container);
...
impl<Ident, Type, Container> WherePredicate<Ident, Type, Container> {
  ...
  #[inline]
  const fn new(span: Span, bounded_type: TypePath<Ident, Type>, bounds: Container) -> Self {
    Self {
      span,
      bounded_type,
      bounds,
  ...
  #[inline]
  pub const fn bounded_type(&self) -> &TypePath<Ident, Type> {
    &self.bounded_type
  }
  ...
  where
    ...
    Container: ChumskyContainer<TypePath<Ident, Type>> + 'a,
