macro_rules! token {
  ($mod:ident $(<$lt:lifetime>)?($slice: ty, $char: ty, $handlers:ident, $source:ty $(,)?)) => {
    mod $mod {
      use logosky::{
        Logos, Lexable, utils::recursion_tracker::{LimitExceeded, Tracker},
      };
      use crate::{
        error::StringErrors,
        lexer::{graphql::{
          ...
          cst::{CstToken, CstTokenKind, tt_hook, tt_hook_and_then, tt_hook_map, increase_recursion_depth_and_token, decrease_recursion_depth},
        }, handlers::*, LitBlockStr, LitInlineStr, SealedWrapper,},
      };

      type TokenError = LexerError<$char, Tracker>;
      type TokenErrors = LexerErrors<$char, Tracker>;
