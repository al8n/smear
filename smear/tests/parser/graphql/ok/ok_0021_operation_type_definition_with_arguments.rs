...
#[test]
fn graphqlx_operation_type_definition_with_arguments() {
...

  {
    let var_other = args.next().unwrap();
    assert_eq!(var_other.variable().name().source(), "varOther");
    let ty = var_other.ty().unwrap_path_ref();
    assert_eq!(ty.path(), "otherInput");
    assert!(!ty.required());
  }
