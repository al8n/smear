...
//! The mutation law for the GraphQL projection: a falsifier for "a walk represents every
//! non-trivia byte, or it refuses".
//!
//! `lossless_x_mutation.rs`'s twin, over this dialect's corpus, doors and oracles. al8n/smear#218's
//! addenda measured, against `graphql/lossless/project.rs`, every hatch the GraphQLx projection
//! closed — a token folded by kind, a child dropped by a wildcard, a name dropped past the third, a
//! shared extension walker taking the union of six tails — and a cell per finding pins the finding.
//! This pins the **class**: it perturbs real trees one child at a time and asks whether the
//! projection still agrees with the syntactic parser, or refuses.
//!
//! # The law
//!
...
//! for trees that are parses. Those carry the no-panic obligation only. Every
//! **skeleton-faithful** one is held to the parser:
//!
...
//!    remapped value — or both must refuse.
//!
...
type SyntaxElement = rowan::NodeOrToken<SyntaxNode, rowan::SyntaxToken<GraphQLLang>>;

/// The six ignorable token images, as `graphql/lossless/project.rs` spells them.
...
///
/// The criterion al8n/smear#58 ruled for GraphQLx and al8n/smear#218 carries here: a position is
/// lenient iff the token has **no AST image of its own** *and* there is a witness that the lossless
/// parser builds the tree **hole-free** without it. Only a token is gone, every constituent the AST
/// holds is still there, and the value is the one the parser builds for the text with the token
/// restored — which is exactly what the law checks.
///
/// Each row carries the token's spelling, because the restoration re-inserts it. **The rows are
/// derived elsewhere** — from the report-and-build census in `lossless_project.rs`, whose every
...
/// missed.
const LENIENT: [(K, K, &str); 13] = [
  // The unclosed-delimiter family — `recover.rs`'s `unclosed_{object,parens,list}`, which report
  // and build no hole.
  (K::SelectionSet, K::RBrace, "}"),
  (K::FieldsDefinition, K::RBrace, "}"),
  (K::InputFieldsDefinition, K::RBrace, "}"),
  (K::EnumValuesDefinition, K::RBrace, "}"),
  (K::RootOperationTypeDefinitions, K::RBrace, "}"),
  (K::Arguments, K::RParen, ")"),
  // The two `on` keywords a production reports missing and builds around: a fragment
  // definition's type condition (`selection.rs`'s `type_condition`) and a directive definition's
  // location list (`definition.rs`'s `directive_definition`). GraphQL's kind space has no
  // `TypeCondition` node, so the first is a token of the fragment definition itself.
  (K::FragmentDefinition, K::Name, "on"),
  (K::DirectiveDefinition, K::Name, "on"),
  // The rest of the `unclosed_*` family.
...
fn oracle_mixed(src: &str) -> Result<Document<&str>, GraphqlErrors<&str>> {
...
fn oracle_executable(src: &str) -> Result<ExecutableDocument<&str>, GraphqlErrors<&str>> {
  Parser::with_parser::<
    '_,
    GraphqlLexer<'_, str>,
...
fn oracle_type_system(src: &str) -> Result<TypeSystemDocument<&str>, GraphqlErrors<&str>> {
  Parser::with_parser::<
    '_,
    GraphqlLexer<'_, str>,
...
fn corpus() -> Vec<(String, String)> {
  ...
  let mut files: Vec<PathBuf> = std::fs::read_dir(&dir)
    .expect("the GraphQL corpus is readable")
...
/// front of the constituent it introduces — the location list for a directive definition's `on`,
/// the type condition's `NamedType` for a fragment definition's.
...
fn restorations(root: &SyntaxNode) -> Vec<(usize, usize)> {
  ...
  for node in root.descendants() {
    for (row, (parent, kind, text)) in LENIENT.iter().enumerate() {
      ...
      // A closer is present when its container holds one. A keyword is read by **position**:
      // both keyword rows' nodes open with two direct `Name`s — the definition's own keyword and
      // its name — and the `on` is a third. In this kind space a definition's name is a bare
      // `Name` token that may itself spell `on` (`directive @on on FIELD`), so a test by spelling
      // alone reads the name as the keyword; GraphQLx's law never meets this because a name there
      // is a node.
      let present = match *kind {
        ...
        _ => node.children_with_tokens().any(|element| match element {
...
///
/// A class appearing here is a finding: name it rather than ledgering it.
...
const LENIENT_BASES: usize = 2;

/// Mutations tried, per door, per operation — the population, cell by cell.
const TRIED: &[((&str, &str), usize)] = &[
  (("executable", "delete"), 748),
  (("executable", "duplicate"), 748),
  (("executable", "retext"), 829),
  (("executable", "swap"), 410),
  (("executable", "transplant"), 7375),
  (("mixed", "delete"), 1682),
  (("mixed", "duplicate"), 1682),
  (("mixed", "retext"), 2389),
  (("mixed", "swap"), 949),
  (("mixed", "transplant"), 16_811),
  (("type-system", "delete"), 876),
  (("type-system", "duplicate"), 876),
  (("type-system", "retext"), 1460),
  (("type-system", "swap"), 504),
  (("type-system", "transplant"), 8392),
];

/// Mutations tried in total.
const TRIED_TOTAL: usize = 45_731;

/// Skeleton-unfaithful: the no-panic obligation only.
const UNFAITHFUL: usize = 39_597;

/// Skeleton-faithful, nothing lenient missing, and the projection equal to the parse.
const EQUAL: usize = 4433;

/// Skeleton-faithful, nothing lenient missing, and both refusing.
const BOTH_REFUSE: usize = 1501;

/// Skeleton-faithful with a lenient token missing, and the projection equal to the restored parse.
const LENIENT_EQUAL: usize = 149;

/// Skeleton-faithful with a lenient token missing, and both refusing the restored text.
const LENIENT_BOTH_REFUSE: usize = 51;

/// How many restorations each [`LENIENT`] row contributed to [`LENIENT_EQUAL`], in table order —
/// the value witnesses this law finds.
///
/// The last five — the closers the report-and-build census adds beyond the corpus's own unclosed
/// brace — have none from this corpus: deleting a closer mid-document re-parses to a different
/// skeleton, and only end of input keeps it faithful. Their values are pinned by
/// `lossless_project.rs`'s `every_unclosed_closer_projects_what_the_closed_text_parses_to` and by
/// `nested_missing_closers_restore_innermost_first` below instead.
const LENIENT_HITS: [usize; 13] = [54, 58, 4, 4, 4, 2, 9, 14, 0, 0, 0, 0, 0];

/// How many restorations each row contributed to [`LENIENT_BOTH_REFUSE`] — restored texts the
/// parser still rejects for another reason, and the projection refuses too.
const LENIENT_REFUSED_HITS: [usize; 13] = [4, 16, 2, 4, 2, 4, 13, 6, 0, 0, 0, 0, 0];
...
/// hole-free without it; a faithful deletion is exactly the second half, so any token here with no
/// image would be a lenient position still strict. Every entry left has an image: a directive
/// location and an operation's name.
...
struct Tally {
  ...
  lenient_hits: [usize; 13],
  lenient_refused_hits: [usize; 13],
...
fn the_projection_agrees_with_the_parser_under_every_one_child_mutation() {
  ...
  // A panicking projection is recorded as a violation below, so the default hook's report of each
  // one is noise; it is put back before any assertion runs.
        ...
        for (slot, &index) in live.iter().enumerate() {
          ...
          for op in &ops {
            *tally
            ...
            // Guarantee 1 is owed to every tree, faithful or not: a panic is a violation in its own
            // right rather than a crash that takes the census with it.
            let projected = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
              door.project(mutated, &text)
            })) {
              Ok(projected) => projected,
              Err(_) => {
                tally.violations.push(std::format!(
                  "{}: the projection panicked on `{text}`",
                  at()
                ));
                continue;
              }
  ...
  let shown = std::env::var("SMEAR_LAW_ALL").map_or(60, |_| usize::MAX);
  for violation in tally.violations.iter().take(shown) {
    println!("  {violation}");
  }
  for (deleted, count) in &tally.strict_deletions {
    println!("STRICT {deleted} {count}");
  }
  ...
  // A row's witness is its report-and-build probe (`lossless_project.rs`'s derivation), not
