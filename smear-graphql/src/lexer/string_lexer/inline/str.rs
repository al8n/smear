...
impl<'a, S, T, StateError> Lexable<&mut SealedWrapper<Lexer<'a, T>>, LexerError<char, StateError>>
...
{
  #[inline]
  fn lex(lexer: &mut SealedWrapper<Lexer<'a, T>>) -> Result<Self, LexerError<char, StateError>>
  where
    Self: Sized,
  {
    lex_inline_str_from_str(lexer).map_err(|errs| LexerError::new(lexer.span(), errs.into()))
  }
}
