...
macro_rules! atom_parser {
  () => {{
    select! {
      ...
      Lexed::Token(Spanned { span, data: AstToken::StringLiteral(raw) }) => Self::String(StringValue::new(span, raw.into())),
      Lexed::Token(Spanned { span, data: AstToken::BlockStringLiteral(raw) }) => Self::String(StringValue::new(span, raw.into())),
    }
  }};
}
