  ...
  Directives: Parseable<'a, I, T, Error> + 'a,
{
    ...
    E: ParserExtra<'a, I, Error = Error> + 'a,
  {
    recursive(|field_parser| {
      ...
      let selection_set = v2::SelectionSet::parser_with(selection).map(SelectionSet);

      v2::Field::parser_with(Arguments::parser(), Directives::parser(), selection_set).map(Self)
    })
  }
}
