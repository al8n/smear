...
use smear_parser::{
  lang::{
    minized::{self, FragmentSpread, InlineFragment, keywords::On},
...
pub type SelectionSet<Alias, Name, FragmentName, TypeCondition, Arguments, Directives> =
  minized::SelectionSet<Selection<Alias, Name, FragmentName, TypeCondition, Arguments, Directives>>;
  ...
  Directives: Parseable<'a, I, T, Error>,
{
    ...
    Error: 'a,
  {
    recursive(|selection| {
      ...
      let field_p = minized::Field::parser_with(
        Arguments::parser(),
        Directives::parser(),
        selsetion_set.clone(),
      )
      ...
      let inline_p = minized::InlineFragment::parser_with(
