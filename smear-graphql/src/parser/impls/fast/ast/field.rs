...
use smear_parser::{
  lang::{
    minized::{self, FragmentSpread, keywords::On},
    ...
    E: ParserExtra<'a, I, Error = Error> + 'a,
  {
    recursive(|field_parser| {
      ...
      let selection = recursive(|selection| {
        ...
        let selection_set = minized::SelectionSet::parser_with(selection.clone());

        let spread = FragmentSpread::parser().map(|fs| Selection::FragmentSpread(fs));

        let inline = minized::InlineFragment::parser_with(
      ...
      let selection_set = minized::SelectionSet::parser_with(selection);

      minized::Field::parser_with(Arguments::parser(), Directives::parser(), selection_set)
        .map(Self)
    })
  }
}
