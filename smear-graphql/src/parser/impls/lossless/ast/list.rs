...
impl<'a, V> Parseable<'a, LosslessTokenStream<'a>, Token<'a>, LosslessTokenErrors<'a, &'a str>> for List<Padded<V, &'a str>>
where
  V: Parseable<
      'a,
...
where
  ...
  V: 'a,
{
  <LBracket as Parseable<'a, LosslessTokenStream<'a>, Token<'a>, LosslessTokenErrors<'a, &'a str>>>::parser()
    .then(padded::padded_parser(value_parser).repeated().collect())
    .then(<RBracket as Parseable<'a, LosslessTokenStream<'a>, Token<'a>, LosslessTokenErrors<'a, &'a str>>>::parser().or_not())
