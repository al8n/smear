...
use crate::{
  error::{Error, Errors},
  parser::{ast::{Colon, LBrace, Name, Object, RBrace}, impls::lossless::padded::Padded},
};
...
impl<InputValue, S> ObjectField<InputValue, S> {
  #[inline]
  pub(crate) const fn new(span: Span, name: Name<S>, colon: Padded<Colon, S>, value: InputValue) -> Self {
    Self {
      span,
      name,
      colon,
    ...
    >,
{
    ...
    E: ParserExtra<'a, TokenStream<'a, Token<'a>>, Error = Self::Error>,
  {
    <Name<&'a str> as Parseable<'a, TokenStream<'a, Token<'a>>>>::parser()
      .then(<Padded<Colon, &'a str> as Parseable<'a, TokenStream<'a, Token<'a>>>>::parser())
      .then(V::parser())
      .map_with(|((name, colon), value), exa| Self::new(exa.span(), name, colon, value))
  }
}

impl<'a, V> Parseable<'a, TokenStream<'a, Token<'a>>> for Object<Padded<ObjectField<V, &'a str>, &'a str>>
where
  V: Parseable<
      'a,
      TokenStream<'a, Token<'a>>,
    ...
    >,
{
    ...
    E: ParserExtra<'a, TokenStream<'a, Token<'a>>, Error = Self::Error>,
  {
    <LBrace as Parseable<'a, TokenStream<'a, Token<'a>>>>::parser()
      .then(
        <Padded<ObjectField<V, &'a str>, &'a str> as Parseable<'a, TokenStream<'a, Token<'a>>>>::parser()
          .repeated()
          .collect(),
      )
...
mod tests {
  ...
  fn test_object_parser() {
    let parser = Object::<Padded<ObjectField<StringValue<&str>, &str>, &str>>::parser::<LosslessParserExtra>();
