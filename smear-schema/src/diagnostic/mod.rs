//! The diagnostic contract: what every error in this crate can be asked, whoever is asking.
//!
//! # The problem this exists for
//!
//! Without it, [`Display`](core::fmt::Display) is the entire escape route from an error, and the
//! rendering strategy is therefore decided inside the library. A consumer wanting `miette`,
//! `ariadne`, `codespan-reporting`, `annotate-snippets`, an LSP `Diagnostic` or a draft §7.1.2
//! response object has to re-derive structure from a formatted sentence, or go without.
//!
//! [`Diagnose`] is the structure, and only the structure. It names no renderer, takes no
//! dependency and is not behind a feature: it is `core` plus `tokora::SimpleSpan`, so a build
//! that reaches an embedded target carries it as readily as one that reaches a server.
//!
...
//!   selection";
//! - zero or more [`PathSegment`]s — draft §7.1.2's coordinate in the *response*, for execution
//!   errors;
...
//! [`Diagnose::path_segment`] is strictly §7.1.2's position in the **response tree**. A
//! compile-time diagnostic answers zero path segments — which is exactly the specification's
//! "cannot be associated with a particular field", so there is no `Option`, no tri-state and
//! nothing for a §7 writer to branch on. A structured coordinate that is *not* a response path —
//! the dotted `User.pet.first` an introspection failure names, say — must not be published
//! through the path, because a writer would then emit `errors[].path` for an error that never had
//! one on the wire.
//!
//! # Why indexed access instead of iterators
//!
//! `fn labels(&self) -> impl Iterator<Item = Label>` and its GAT spelling both make [`Diagnose`]
//! dyn-incompatible, and `&dyn Diagnose` is the entire point: a server holds schema refusals,
//! validation diagnostics and execution errors in one rendering pass, and boxing each one to get
//! there would allocate for a value that was designed not to. A count plus an indexed accessor is
//! dyn-safe, allocation-free, and hands a renderer its length up front — which is what an LSP
//! `relatedInformation` array and a `miette` label vector both want before they start filling.
//!
...
//! `tests/validator_allocation.rs` measures that rather than asserting it.
...
/// error's spans mean nothing without a schema to interpret them against, the type that pairs the
/// two is the implementor; where the error is self-contained, the error is. A collection of
/// errors is not a diagnostic and should not pretend to be one: its elements are.
///
...
pub trait Diagnose: core::fmt::Display {
  ...
  /// Returns how many response-path segments the diagnostic carries.
  ///
  /// Zero is the answer for every diagnostic that is about a document rather than a response, and
  /// it is draft §7.1.2's own "cannot be associated with a particular field" rather than a
  /// stand-in for it.
  fn path_segments(&self) -> usize;

  /// Returns the `index`th response-path segment, or `None` past the end.
...
pub trait DiagnoseExt: Diagnose {
  ...
  /// Iterates the response-path segments, root first.
