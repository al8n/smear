  ...
  ObjectTypeExtension<Name, ImplementInterfaces, Directives, FieldsDefinition>
{
    ...
    IP: Parser<'src, I, ImplementInterfaces, E> + Clone + 'src,
  {
    name_parser
      ...
      .then(directives_parser.or_not())
      .then(fields_definition_parser.or_not())
      .try_map_with(|(((name, implements), directives), fields), _exa| {
        let data = match (implements, directives, fields) {
          ...
          (None, None, None) => {
            // TODO: This should be caught by parser - construct IncompleteSyntax if needed
            unreachable!("Object type extension requires implements, directives, or fields definition");
          }
        };
...
/// Implements `AstNode` for `ObjectTypeDefinition` in the GraphQL language.
///
...
/// Implements `AstNode` for `ObjectTypeExtension` in the GraphQL language.
///
