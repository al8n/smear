...
impl<S, Ty> IntoComponents for ObjectTypeDefinition<S, Ty> {
  ...
  fn into_components(self) -> Self::Components {
    ...
    let (where_clause, fields) = match fields {
      Some(fd) => { let (_, wc, f) = fd.into_components(); (wc, Some(f)) }
      None => (None, None),
    };
...
impl<S, Ty> IntoComponents for ObjectTypeExtension<S, Ty> {
  ...
  fn into_components(self) -> Self::Components {
    ...
    match data {
      scaffold::ObjectTypeExtensionData::Directives { implements, directives } => (span, path, generics, implements, Some(directives), None, None),
      scaffold::ObjectTypeExtensionData::Fields { implements, directives, fields } => {
        let (_, wc, f) = fields.into_components();
        (span, path, generics, implements, directives, wc, Some(f))
      }
      scaffold::ObjectTypeExtensionData::Implements(impls) => (span, path, generics, Some(impls), None, None, None),
    }
  }
}
