...
impl<VariableDefinition, Span, Container> VariablesDefinition<VariableDefinition, Span, Container> {
  ...
  pub fn parser_with<'src, I, E, P>(variable_definition_parser: P) -> impl Parser<'src, I, Self, E> + Clone
  where
    I: Source<'src>,
    I::Token: Char + 'src,
    I::Slice: Slice<Token = I::Token>,
    ...
    P: Parser<'src, I, VariableDefinition, E> + Clone,
  {
    LParen::parser()
      .then_ignore(ignored())
      .then(variable_definition_parser.padded_by(ignored()).repeated().collect())
      .then_ignore(ignored())
      .then(RParen::parser())
