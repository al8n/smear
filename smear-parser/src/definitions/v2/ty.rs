...
impl<Name> IntoComponents for NamedType<Name> {
  type Components = (Span, Name, Option<Bang>);
...
impl<Name> NamedType<Name> {
  ...
  #[inline]
  pub const fn bang(&self) -> Option<&Bang> {
    self.bang.as_ref()
  }
...
impl<Type> IntoComponents for ListType<Type> {
  type Components = (Span, LBracket, Type, RBracket, Option<Bang>);
...
impl<Type> ListType<Type> {
    ...
    P: Parser<'a, I, Type, E> + Clone,
  {
    LBracket::parser()
      ...
      .then(Bang::parser().or_not())
      .map_with(|(((l_bracket, ty), r_bracket), bang), exa| Self {
        span: exa.span(),
        l_bracket,
        ty,
        r_bracket,
