...
use crate::lang::{keywords2::{Enum, Extend}, punctuator::{LBrace, RBrace}};
...
impl<EnumValue, Directives> EnumValueDefinition<EnumValue, Directives> {
  ...
  #[inline]
  pub fn parser_with<'src, I, T, Error, E, EP, DP>(enum_value_parser: EP, directives_parser: DP) -> impl Parser<'src, I, Self, E> + Clone
  where
    T: Token<'src>,
...
impl<EnumValueDefinition, Container>
  EnumValuesDefinition<EnumValueDefinition, Container>
{
  ...
  /// [ignored tokens]: https://spec.graphql.org/draft/#sec-Language.Source-Text.Ignored-Tokens
  #[inline]
  pub fn parser_with<'src, I, T, Error, E, P>(enum_value_parser: P) -> impl Parser<'src, I, Self, E> + Clone
  where
    T: Token<'src>,
    I: Tokenizer<'src, T, Slice = <T::Source as Source>::Slice<'src>>,
    Error: 'src,
    ...
    Container: chumsky::container::Container<EnumValueDefinition>,
  {
    LBrace::parser()
      ...
      .then(RBrace::parser())
      .map_with(|((l_brace, enum_values), r_brace), exa| Self {
        span: exa.span(),
        l_brace,
        r_brace,
        enum_values,
...
impl<'a, Name, Directives, EnumValuesDefinition, I, T, Error>
  ...
  EnumValuesDefinition: Parseable<'a, I, T, Error>,
{
  ...
  where
    ...
    E: ParserExtra<'a, I, Error = Error> + 'a,
  {
    Self::parser_with(Name::parser(), Directives::parser(), EnumValuesDefinition::parser())
  }
}
  ...
  EnumTypeDefinition<Name, Directives, EnumValuesDefinition>
{
  ...
  #[inline]
  pub const fn enum_keyword(&self) -> &Enum {
    &self.keyword
  }
  ...
  {
    Enum::parser()
      .then(EnumTypeDefinitionContent::parser_with(
        name_parser,
        directives_parser,
        enum_values_definition_parser,
      ))
      .map_with(|(keyword, content), exa| {
  ...
  EnumTypeExtension<Name, Directives, EnumValuesDefinition>
{
    ...
    DP: Parser<'src, I, Directives, E> + Clone,
  {
    Extend::parser()
      ...
      .map_with(|((extend, keyword), data), exa| {
