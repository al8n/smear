...
use crate::lang::keywords2::{Extend, Scalar};
...
impl<Name, Directives> IntoComponents for ScalarTypeDefinition<Name, Directives> {
  type Components = (Span, Scalar, Name, Option<Directives>);

  #[inline]
  fn into_components(self) -> Self::Components {
    (self.span, self.scalar, self.name, self.directives)
  }
}
...
impl<Name, Directives> ScalarTypeDefinition<Name, Directives> {
  ...
  #[inline]
  pub const fn scalar_keyword(&self) -> &Scalar {
    &self.scalar
  }
...
impl<Name, Directives> IntoComponents for ScalarTypeExtension<Name, Directives> {
  type Components = (Span, Extend, Scalar, Name, Directives);
...
impl<Name, Directives> ScalarTypeExtension<Name, Directives> {
    ...
    DP: Parser<'src, I, Directives, E> + Clone,
  {
    Extend::parser()
      .then(Scalar::parser())
      .then(ScalarTypeExtensionContent::<Name, Directives>::parser_with(
        name_parser,
        directives_parser,
      ))
      .map_with(|((extend, scalar), content), exa| {
        let (_, name, directives) = content.into_components();
  ...
  Directives: Parseable<'a, I, T, Error>,
{
    ...
    E: ParserExtra<'a, I, Error = Error> + 'a,
  {
    Extend::parser()
      .then(Scalar::parser())
      .then(ScalarTypeExtensionContent::<Name, Directives>::parser())
      .map_with(|((extend, scalar), content), exa| {
        let (_, name, directives) = content.into_components();
