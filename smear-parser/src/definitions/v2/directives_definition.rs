...
use crate::{keyword, lang::{punctuator::{Pipe, At}, v2::keyword::{Directive, On, Repeatable}}};
...
impl<Name, Args, Locations> IntoSpan<Span>
...
impl<Name, Args, Locations> IntoComponents
  for DirectiveDefinition<Name, Args, Locations>
{
  type Components = (
    Span,
...
impl<Name, Args, Locations> DirectiveDefinition<Name, Args, Locations> {
  ...
  #[inline]
  pub const fn directive_keyword(&self) -> &Directive {
    &self.keyword
  }
  ...
  #[inline]
  pub const fn repeatable(&self) -> Option<&Repeatable> {
  ...
  #[inline]
  pub const fn on_keyword(&self) -> &On {
    &self.on
  }
  ...
  {
    // 'directive' ~ '@' ~ name ~ arguments_definition? ~ repeatable? ~ 'on' ~ directive_locations
    Directive::parser()
      .then(At::parser())
      .then(name_parser)
      ...
      .map_with(
        |(
          (((((keyword, at), name), arguments_definition), repeateable), on),
          directive_locations,
        ),
        exa| {
          Self {
