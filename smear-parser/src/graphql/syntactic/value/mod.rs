...
/// width its `MaterializedInt` bound admits — are then the same
...
/// The materialised-number instantiation of every production in this module, at each width its
/// `MaterializedInt` bound admits: [`i32`], which draft §3.5.1 specifies `Int` to be, and [`i64`],
/// the reading that accepts every literal the grammar admits.
///
/// A code span rather than a link because that bound is **not exported**: an out-of-crate caller
/// names the width by writing `i32` or `i64` at the call, never by naming the trait. The comment
/// on that module's import of it has the argument.
