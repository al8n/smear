...
//!
//! This crate provides complete parser implementations for both standard GraphQL (draft specification)
//! and GraphQLX (extended GraphQL with generics, imports, and more).
//!
//! # Features
//!
//! - **GraphQL Parsing**: Full draft spec compliance for type systems and executable documents
//! - **GraphQLX Parsing**: Extended syntax with imports, generics, where clauses, map/set types
//! - **Zero-Copy**: All AST nodes reference the original source without allocations
//! - **Generic Over Source Types**: Works with `&str`, `&[u8]`, `bytes::Bytes`, `hipstr` types
//! - **Comprehensive Error Reporting**: Detailed parse errors with source locations
...
//! # Architecture
//!
//! The parser is built on top of two foundational crates:
//!
//! - **`smear-lexer`**: High-performance tokenization with zero-copy tokens
//! - **`smear-scaffold`**: Generic, reusable AST node definitions
//!
//! This separation enables:
//! - Fast compilation times (each crate compiles independently)
//! - Code reuse (scaffold can be used for custom DSLs)
//! - Flexible parsing (choose syntactic or lossless tokens)
//!
//! # Feature Flags
//!
...
//! | `graphqlx` | Extended GraphQL parser (requires `unstable`) | ✓ |
