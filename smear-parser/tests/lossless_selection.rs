...
//! Task 3's stub, so nothing under `parse_str` reaches a selection yet; written the other way
//! every assertion here would compare one empty tree against another and pass without a single
//! production existing. That is the vacuous-assertion failure mode Task 5 recorded, and the
//! reason `value.rs` grew its own drivers first.
//!
