...
#[logos(crate = tokora::logos, error(StringError<char>))]
...
#[derive(Logos, Debug)]
#[logos(crate = tokora::logos, extras = BlockLineExtras)]
enum BlockLineTok {
...
mod tests {
  ...
  fn parity_kitchen_sink_block_strings() {
    ...
    // `smear-lexer/tests/fixtures/…` is a SYMLINK to `smear/tests/fixtures/…`, which is the same
    // device the workspace already uses for `smear/README.md`: cargo materialises a symlink's
    // target into the `.crate` archive, so this package is still self-contained when packaged
    // while the workspace keeps exactly one copy of the corpus for the four other targets that
    // read it. A copy would need a byte-identity gate to stop the two drifting.
    const SRC: &str =
