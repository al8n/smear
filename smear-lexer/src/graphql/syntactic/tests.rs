...
{
  let query = SyntacticToken::Identifier(query);
  assert!(query.is_query());

  let mutation = SyntacticToken::Identifier(mutation);
  assert!(mutation.is_mutation());

  let subscription = SyntacticToken::Identifier(subscription);
  assert!(subscription.is_subscription());

  let fragment = SyntacticToken::Identifier(fragment);
  assert!(fragment.is_fragment());

  let other = SyntacticToken::Identifier(other);
  assert!(
    !other.is_query() && !other.is_mutation() && !other.is_subscription() && !other.is_fragment()
  );

  let punctuator: SyntacticToken<S> = SyntacticToken::LBrace;
  assert!(
    !punctuator.is_query()
