...
///
/// Not private since #58: the CST-to-AST projection reads a keyword off a tree token's text and
/// must classify it exactly as the lexer did, and a second copy of this table in the projection
/// would be a spelling the two layers could disagree about. It was `pub(crate)` while the
/// projection lived in this crate; the projection is `smear-parser`'s now, so the same one
/// consumer reaches it across a crate boundary and the visibility follows.
#[inline]
