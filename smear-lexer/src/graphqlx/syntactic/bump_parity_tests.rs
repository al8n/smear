//! `Lexer::bump` parity for the GraphQLx SIMD lexer, mirroring
//! `graphql/syntactic/bump_parity_tests.rs`.
//!
...
use crate::graphqlx::syntactic::{SyntacticLexer, SyntacticToken};
...
fn bump_after_error_token_matches_logos() {
  ...
  // stays a valid boundary. Frozen reference: the error token's `Debug` text
  // is exactly the constant below; after `bump(1)` the span grows to 0..3 and
  // the slice becomes "..x".
  let src = "..x";
  let mut simd = SyntacticLexer::<str>::new(src);
...
fn bump_past_end_panics_like_logos() {
  // Whole source consumed, then a one-byte bump lands past the end.
  // `logos::Lexer::bump` asserts `is_boundary(token_end)` (`index <= len` for a
  // `str`'s byte length), which fails unconditionally here; the SIMD lexer must
...
fn bump_into_multibyte_char_panics_like_logos() {
  ...
  // `str`, `logos::Lexer::bump` asserts `is_char_boundary` and panics
  // unconditionally here; the SIMD lexer must too.
  let src = "aé";
  let simd_panicked = panics(|| {
    let mut simd = SyntacticLexer::<str>::new(src);
    let _ = simd.lex();
